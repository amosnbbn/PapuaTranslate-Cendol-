...
def main():
    ap = argparse.ArgumentParser("Fine-tune mT5 dari JSON splits (instruction/input/output)")
    ap.add_argument("--base_dir", required=True)
    ap.add_argument("--adapter_init", default="", help="Folder checkpoint LoRA untuk melanjutkan")
    ap.add_argument("--splits_dir", required=True, help="Folder berisi train.json, valid.json, test.json")
    ap.add_argument("--out_dir", required=True)

    # ==== default untuk fase 7 (10 epoch) ====
    ...
    ap.add_argument("--fp16", action="store_true", help="Aktifkan FP16 (default FP32)")
    ap.add_argument("--bf16", action="store_true", help="Aktifkan BF16 bila GPU mendukung")
    args = ap.parse_args()
    ...
    # === Load dataset JSON (instruction,input,output) ===
    files = {
    ...
    # === Load LoRA adapter utk melanjutkan fine-tune ===
    if args.adapter_init and os.path.isdir(args.adapter_init):
        ...
        # Tanpa adapter_init -> tetap jalan, tapi ini bukan LoRA-continue.
        # Disarankan isi adapter_init supaya cuma LoRA yang dilatih.
        model = base
    model.train()
    ...
    # === Training args: 10 epoch + cosine scheduler + early stopping ===
